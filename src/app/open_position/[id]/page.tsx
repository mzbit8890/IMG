...
import linked from "@/app/open_position/assets/linkedin.png"
import face from "@/app/open_position/assets/facebook (1).png"
import twit from "@/app/open_position/assets/twitter.png"
import hash from "@/app/open_position/assets/hash.png"
import map from "@/app/open_position/assets/map-pin (1).png"
import logo99 from "@/../../public/IMG-Logo-4.png";
import category from "@/app/open_position/assets/menu.png"
...
const JobDetails = ({ params }: { params: { id: string } }) => {
    const job: JobBox[] = JobBox
    return (
        <>

            <Image src={meeting} alt="Meeting" className='relative -top-28 z-6 h-[570px] object-cover brightness-50' />
            <Wrapper>
                <div className='absolute inset-0 mt-20 h-[1px] bg-gray-500 w-full flex '></div>

                <div className=" max-w-screen-2xl mx-auto px-3 job6:px-6 xl:px-9 absolute inset-0  sm:flex pt-44 sm:pt-64 h-[90%]">

                    <div className='flex  gap-2 sm:gap-4 mr-4 job2:mr-5 my-3'>
                        <div className='h-16 w-16 job3:h-20 job3:w-20'>
                            <div className='flex rounded-full  h-16 w-16 job3:h-20 job3:w-20  justify-center items-center bg-gray-700'>
                                <Image src={logo99} alt="Img Logo" className='w-8 h-8 job3:h-10 job3:w-10' />
                            </div>
                        </div>

                        <div className='my-1'>
                            <h2 className='text-slate-200 font-semibold text-[18px]'>IMG, INC</h2>
                            <div>
                                {
                                    JobBox.map((item,key) => {
                                        return (

                                            <div key={item.JOBId}>{item.JOBIdPath == params.id ? (
                                                <div className='flex  items-center gap-1 mt-3'>
                                                    <Image src={category} alt="Map Icon" className='w-3 opacity-100 brightness-200 h-3' /><p className='sm:w-36 job3:w-44 xl:w-60 text-white/70 text-[13px] job:text-[14px]'>{item.category}</p>
                                                </div>
                                            ) : ""}
                                            </div>
                            ...
                            JobBox.map((item,key) => {
                                return (
                                    <div key={item.JOBId} className='flex  '>
                                        <div>{item.JOBIdPath == params.id ? (
                                            <>

                                                <div className='flex gap-5 md:gap-9 job:gap-12'>
                                                    <div className='sm:flex hidden items-stretch'><div className=' w-[1px]  bg-gray-500 '></div></div>

                                                    <div className='my-3'>
                                                        <h1 className='text-slate-200 text-[16px] job5:text-[19px] md:text-[23px] xl:text-[25px]'>{item.head}</h1>
                                                        <div className='text-white/70 text-[12px] job6:text-[14px] flex  mt-5'>
                                                            <div className='flex gap-1 items-center mr-4 job6:mr-8'><Image src={map} alt="Map Icon" className='w-4 h-4' /><p>{item.Loc}</p></div>
                                                            <div className='flex gap-1 items-center'><Image src={hash} alt="Hash Icon" className='w-4 h-4' /><p>{item.noOfOpenings}</p></div>
                ...
                <div className='relative  -top-24 flex gap-8 '>
                    <div className=' rounded-lg -top-24 w-[75%]  border-2 border-black/5 shadow-md bg-white/55 '>

                    </div>
                    <div className='  rounded-lg  w-[25%] bg-black/5  shadow-md '>
                        <div className='flex flex-col px-3 items-center'>
                            <button className='bg-blue-300 text-gray-700 rounded-[32px] px-10 py-5  mt-20'>
                                Apply for this Job online
                            </button>
                            <button className='bg-green-300 text-gray-700 rounded-[32px] px-10 py-5  mt-5'>
                                Email this Job online to a friend
                            </button>
                            <p className='text-[23px] text-gray-600 my-10'>Or</p>
                            <p className=' text-gray-600 text-lg font-semibold'>Share on your newsfeed</p>
                            <div className='flex items-center mt-5 gap-4 mb-20'>

                                <Image src={face} alt="Facebook icon" className=' w-[48px] h-[48px]' />
                                <Image src={linked} alt="Linked icon" className=' w-[60px] h-[60px]' />
                                <Image src={twit} alt="Twitter icon" className='w-[48px] h-[48px]' />
                ...
                </div><div className='relative px-10 pt-10 pb-20 -top-16    border-2 border-black/5 shadow-md flex flex-col  rounded-lg w-[73%] '>
                    <div >
                        <h2 className='text-3xl font-bold '>About IMG</h2>
                        <div className='flex mt-10 items-center gap-4'>
                        <div className='h-16 w-16 job3:h-20 job3:w-20'>
                            
                                <div className='flex  rounded-full  h-16 w-16 job3:h-20 job3:w-20  justify-center items-center border-2'>
                                    <Image src={logo99} alt="Img Logo" className='w-8 h-8 job3:h-10 job3:w-10' />
                                </div></div>
                                <span className="text-blue-600 text-2xl font-bold  tracking-wider  "><span className="py-1 pl-5  bg-blue-100">Information Man</span>agement Group, Inc</span>
                                

                        </div>
                        {/* <p className='text-[15px] mt-6 text-gray-800'>
                        </p> */}
                        <div>
                        {
                            JobBox.map((item,key) => {
                                return (
                                        ...
                                        <div key={item.JOBId}>{item.JOBIdPath == params.id ? (
                                           <p className='text-[15px] font-[500] mt-6 text-gray-800'>{item.aboutImg}</p>
                                        ) : ""}
                                        </div>
                                )
                            })
...
<div className="  text-center pb-9  relative mt-8">
